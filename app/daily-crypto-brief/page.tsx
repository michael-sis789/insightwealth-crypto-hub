  ...
  return (
    <div className="container-shell py-10">
      ...
      <section className="mt-8 grid gap-6 lg:grid-cols-[1fr_360px]">
        ...
        <YouTubePanel title="今日影片與頻道" compact />
      </section>
      <section className="premium-card mt-8 rounded-lg p-6">
        <h2 className="text-2xl font-bold text-white">今天長期BTC投資者該看什麼？</h2>
        <div className="mt-4 grid gap-4 md:grid-cols-3">
          <div className="rounded border border-line p-4">
            <p className="font-bold text-gold">1. 價格不是結論</p>
            <p className="mt-2 text-sm leading-6 text-slate-400">先看BTC價格與24h變化，再確認ETF資金流、情緒與槓桿是否支持同一方向。</p>
          </div>
          <div className="rounded border border-line p-4">
            <p className="font-bold text-gold">2. DCA優先於猜頂底</p>
            <p className="mt-2 text-sm leading-6 text-slate-400">每日分數只用來調整節奏，不用來做all-in決策。長期策略應該可重複、可承受、可紀律執行。</p>
          </div>
          <div className="rounded border border-line p-4">
            <p className="font-bold text-gold">3. 風險管理放前面</p>
            <p className="mt-2 text-sm leading-6 text-slate-400">若市場波動加大，先檢查現金流、倉位與時間週期，而不是被單日新聞牽著走。</p>
          </div>
        </div>
      </section>
      ...
      <SeoContent title="Daily Crypto Brief如何支持每日影片？" paragraphs={[
        ...
        "影片與網站的關係是互相補強：YouTube負責快速解說與建立信任，網站負責保留可搜尋、可引用、可回看的文字內容。當觀眾想重新檢查數據來源、DCA觀點或相關工具時，可以回到Daily Crypto Brief頁面，而不是只依賴影片時間線。",
